      ...
      <div className="absolute inset-0 grid place-items-center px-6">
        <SurfaceCard variant="outglow" className={`w-full ${maxWidthClassName} p-6`}>
          ...
          <div className="mt-6 flex gap-4">
            <a
              href={cancelHref}
              className="flex-1 basis-0 rounded-xl bg-white px-4 py-3 text-center text-[15px] font-semibold text-[#0B0C10] whitespace-nowrap"
            >
              {cancelText}
            </a>
            ...
            {onConfirm ? (
              <button
                type="button"
                onClick={onConfirm}
                className="flex-1 basis-0 rounded-xl bg-panana-pink px-4 py-3 text-center text-[15px] font-semibold text-white whitespace-nowrap"
              >
                {confirmText}
              </button>
            ) : (
