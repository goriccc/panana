...
/** 유저(마이)의 팔로워/팔로잉 목록. 팔로워=나를 팔로우하는 캐릭터, 팔로잉=내가 팔로우한 캐릭터. */
export async function GET(req: Request) {
  try {
    ...
    if (tab === "followers") {
      ...
      slugs = [...new Set((rows || []).map((r: any) => String(r.character_slug || "").trim().toLowerCase()).filter(Boolean))];
    } else {
      const { data: rows, error } = await sb
    ...
    const listWithFollowing = list.map((c) => ({
      ...c,
