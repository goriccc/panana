...
export async function GET(req: NextRequest) {
  try {
    ...
    const systemPrompt = `${baseSystem}\n\n[음성 대화 - 필수]\n- **절대 지문을 읽지 않는다.** 괄호 (), （） 안의 행동·상황 묘사는 대사가 아니다. 화면에만 표시되고 음성으로는 절대 출력하지 않는다.\n- "(미소 지으며)", "(한숨)", "(돌아보며)" 등 어떤 괄호 표현도 읽지 말고 무시한다.\n- 음성 출력은 반드시 "말하는 대사"만 한다. 지문이 섞여 있으면 대사 부분만 골라서 말한다.\n- 발화 직전 자체 검증: 생성 문장에 괄호 텍스트가 포함되면 괄호 구간을 제거한 뒤 대사만 다시 발화한다.\n- ${styleText}\n- 음성으로 자연스럽게 대화한다. 짧은 문장 위주로 말한다.`;
