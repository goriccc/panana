...
export async function POST(
  ...
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as { pananaId?: string; startedAt?: string };
    ...
    const durationMs = startedAt ? Math.max(0, completedAt.getTime() - startedAt.getTime()) : 0;
    ...
    const actualStartedAt = sess?.started_at ? new Date(sess.started_at) : startedAt || completedAt;
    const actualDurationMs = Math.max(0, completedAt.getTime() - actualStartedAt.getTime());

    const { error: insertErr } = await sb.from("panana_challenge_completions").insert({
      user_id: userId,
