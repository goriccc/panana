...
export function TriggerBuilderClient({ characterId }: { characterId: string }) {
    ...
    <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
      <aside className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        ...
        <div className="mt-3 space-y-2">
          <a className="block rounded-xl bg-white/[0.04] px-4 py-3 text-[13px] font-extrabold text-white/80">
            변수 트리거
          </a>
          <a className="block rounded-xl bg-white/[0.02] px-4 py-3 text-[13px] font-semibold text-white/55">
            프롬프트 에디터
          </a>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    className="rounded-lg bg-white/[0.03] px-3 py-2 text-[12px] font-extrabold text-white/55 ring-1 ring-white/10 hover:bg-white/[0.05]"
                    onClick={() => setTriggers(characterId, { rules: rules.filter((x) => x.id !== r.id) })}
                  >
                    🗑
                  </button>
                </div>
                        ...
                        {a.type === "variable_mod" ? (
                          <ActionRow label="결과 유형: 변수 값 변경">
                            <select
                              ...
                              onChange={(e) => {
                                const nextActions = r.then.actions.map((x, i) =>
                                  i === aIdx ? ({ ...a, var: e.target.value } as any) : x
                                );
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                const nextActions = r.then.actions.map((x, i) =>
                                  i === aIdx ? ({ ...a, op: e.target.value } as any) : x
                                );
                                updateRule(r.id, { then: { actions: nextActions } });
                              }}
                              className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-[12px] font-extrabold text-white/75 outline-none"
                        ...
                        {a.type === "system_message" ? (
                          <ActionRow label="결과 유형: 시스템 메시지 출력">
                            <input
                              ...
                              onChange={(e) => {
                                const nextActions = r.then.actions.map((x, i) =>
                                  i === aIdx ? ({ ...a, text: e.target.value } as any) : x
                                );
                                updateRule(r.id, { then: { actions: nextActions } });
                              }}
                        ...
                        {a.type === "status_effect" ? (
                          <ActionRow label="결과 유형: 특수 상태 부여">
                            <input
                              ...
                              onChange={(e) => {
                                const nextActions = r.then.actions.map((x, i) =>
                                  i === aIdx ? ({ ...a, key: e.target.value } as any) : x
                                );
                                updateRule(r.id, { then: { actions: nextActions } });
                              }}
